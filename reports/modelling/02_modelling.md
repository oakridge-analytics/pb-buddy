...
input_blob_name = "2024-07-14_22_00_53__adjusted_bike_ads.parquet.gzip"
```
...
```python
X_train_outlier_scored = df_modelling.assign(
...
```python
(X_train_outlier_scored.outlier_flag.value_counts())
```
...
(
    ...
    .assign(url=lambda _df: _df.url.apply(make_clickable))
    .style
)
```
...
```python
results["baseline"] = pd.DataFrame(base_grid_search.cv_results_).drop(
    columns=[c for c in base_grid_search.cv_results_.keys() if "param_" in c]
)
...
def predefined_grid_search(
    ...
    **kwargs,
    ...
    for train_set_ix, _ in cv.split():
        X_train, y_train = (
            X_train_valid.iloc[train_set_ix],
            y_train_valid.iloc[train_set_ix],
        )
...
hparams = {
    ...
    "transform__preprocess__description_text__tfidf__ngram_range": [
        (1, 3),
        (1, 5),
        (2, 5),
    ],
...
hparams = {
    ...
    "transform__preprocess__title_text__tfidf__ngram_range": [
        (1, 3),
    ],
...
```python
mode_msrp = df_valid_gluon.specs_data_numerical__msrp_cleaned.mode().iloc[0]
initial_df = (
    df_valid_gluon.query("specs_data_numerical__msrp_cleaned != @mode_msrp")
    .sample(2)
    .assign(pred=lambda _df: predictor.predict(_df))
    .head(1)
)
...
plt.plot(
    initial_df["add_age__age_at_post"].iloc[0],
    initial_df["price__price_cpi_adjusted_CAD"].iloc[0],
    "go",
)

display(initial_df.head(1))
...
for row in initial_df[["description_text__description"]].head(1).itertuples():
    print("\n".join(textwrap.wrap(row.description_text__description, width=width)))
```
...
PartialDependenceDisplay.from_estimator(
    predictor,
    df_valid_gluon.sample(10000),
    [
        "add_age__age_at_post",
        ("add_age__age_at_post", "specs_data_numerical__msrp_cleaned"),
    ],
    grid_resolution=10,
    verbose=4,
    ax=ax,
...
sample_df
sample_df.plot(
    x="add_post_month__original_post_date",
    y="pred",
    title=sample_df["title_text__ad_title"].iloc[0],
    marker="o",
)
...
(
    df_gluon_inspection.assign(abs_resid=lambda _df: np.abs(_df.resid))[
        [
            "url",
            "add_age__age_at_post",
            "original_post_date",
    ...
    .head(20)
    .assign(url=lambda _df: _df.url.apply(make_clickable))
    .style.background_gradient(subset="resid", cmap="RdBu")
...
for col in check_cols:
    g = (
        ...
        .assign(**{f"{col}__isna": lambda _df: (_df[col].isna()) | (_df[col] == 0.0)})
...
```python
g = df_gluon_inspection.astype({"price__price_cpi_adjusted_CAD": float}).pipe(
    (sns.jointplot, "data"),
    kind="reg",
    y="pred",
    x="price__price_cpi_adjusted_CAD",
...
g = (
    ...
    .assign(years_old=lambda _df: _df.add_age__age_at_post / 365)
    .sample(10000)
    .pipe(
        (sns.jointplot, "data"),
        x="years_old",
        y="price__price_cpi_adjusted_CAD",
        height=10,
    )
)
...
ad_url = "https://www.pinkbike.com/buysell/3892662/"
...
old_pred = predictor_old.predict(ad_data, as_pandas=False)
new_pred = predictor.predict(ad_data, as_pandas=False)
